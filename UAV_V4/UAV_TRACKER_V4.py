...
# GÜNCELLENDİ: BoT-SORT'un "hayalet" hedefleri daha çabuk unutması ve zor kilitlenmesi için ayarlar sıkılaştırıldı
yaml_dosyasi = "TRACKER_V4_botsort.yaml"
...
print(f" [+] '{yaml_dosyasi}' Ultralytics formatinda ayarlandi!")

print("GPU (Ekran Karti) Aktif mi?:", torch.cuda.is_available())

print("\n" + "═"*50)
print(" ✈️  TEKNOFEST SAVAŞAN İHA - BOT-SORT + KALMAN SİSTEMİ ")
print("═"*50)
...
# ══════════════════════════════════════════════════════════════════════════════
#  HİBRİT TAKİP SINIFI (BOT-SORT + LK + KALMAN)
# ══════════════════════════════════════════════════════════════════════════════
class BotSortHibrid:
    ...
    def guncelle(self, frame, gray):
        results = self.model.track(frame, persist=True, tracker="TRACKER_V4_botsort.yaml", conf=CONF_THRESH, verbose=False)
...
while True:
    ...
    mod_renk = (0, 255, 0) if takipci.mod == takipci.MOD_BOT else (0, 165, 255)
